...
def load_week_from_db(user_id: int, is_admin: bool = False, semana: int = None, anio: int = None) -> pd.DataFrame:
    ...
    query = """
        SELECT 
            ...
            'Vacas masa' as "CONCEPTO", ds.vacas_masa as "A. TOTAL"
        ...
        JOIN establecimientos est ON ds.establecimiento_id = est.id
        JOIN empresas emp ON ds.empresa_id = emp.id
        WHERE ds.semana = %s AND ds.anio = %s AND ds.praderas_otros_verdes IS NOT NULL
    ...
    # Repetir parámetros para cada UNION
    params = tuple([semana, anio] * 17)  # 17 conceptos
...
def load_historic_from_db(user_id: int = None, is_admin: bool = False) -> pd.DataFrame:
    ...
    query = """
        SELECT 
            NULL as "Fecha",
            hm.semana as "N° Semana",
            emp.nombre as "Empresa",
            est.nombre as "Establecimiento",
            hm.mdat as "MDAT",
            hm.vacas_en_ordena as "Vacas en ordeña"
        FROM historico_mdat hm
        JOIN establecimientos est ON hm.establecimiento_id = est.id
        JOIN empresas emp ON hm.empresa_id = emp.id
        ORDER BY hm.semana DESC
    """
